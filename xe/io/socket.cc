...
void xe_socket::init_fd(int _fd){
	fd = _fd;
}
...
int xe_socket::connect(sockaddr* addr, socklen_t addrlen, ulong key){
	if(flags & (XE_SOCKET_CONNECTING | XE_SOCKET_ACCEPTING | XE_SOCKET_CONNECTED))
		return XE_EINVAL;
	int ret = loop.connect(fd, addr, addrlen, this, null, XE_SOCKET_CONNECT, key, XE_HANDLE_SOCKET);
...
int xe_socket::recv(xe_buf buf, uint len, uint flags, ulong key){
	return loop.recv(fd, buf, len, flags, this, null, XE_SOCKET_RECV, key, XE_HANDLE_SOCKET);
}

int xe_socket::send(xe_buf buf, uint len, uint flags, ulong key){
	return loop.send(fd, buf, len, flags, this, null, XE_SOCKET_SEND, key, XE_HANDLE_SOCKET);
}
...
int xe_socket::accept(sockaddr* addr, socklen_t* addrlen, uint flags){
	if(flags & (XE_SOCKET_CONNECTING | XE_SOCKET_CONNECTED | XE_SOCKET_ACCEPTING))
		return XE_EINVAL;
	int ret = loop.accept(fd, addr, addrlen, flags, this, null, XE_SOCKET_ACCEPT, 0, XE_HANDLE_SOCKET);
...
void xe_socket::io(xe_handle& handle, int result){
